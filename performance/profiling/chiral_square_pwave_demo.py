...
def chiral_square_problem():
    ...
    return model, guess, integration, scf


def solve_chiral_square_state():
    model, guess, integration, scf = chiral_square_problem()
    result = solver(
        model,
        guess,
...
def write_chiral_square_plot(output_path: str | Path, *, nk: int = 301) -> Path:
...
def main() -> None:
    ...
    parser.add_argument("--nk", type=int, default=301)
    args = parser.parse_args()

    path = write_chiral_square_plot(args.output, nk=args.nk)
    print(path)
