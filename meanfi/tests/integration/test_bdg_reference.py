...
def test_bdg_reference_energy_and_hamiltonian(
    ...
    with monkeypatch.context() as patch:
        ...
        error = abs(mf.internal_energy(model, density) - expected_energy)
        assert error < 1e-14, f"BdG reference energy error: {error}"
        assert mf.free_energy(model, density) == pytest.approx(
    ...
    step = 1e-5
    derivative = (
        mf.internal_energy(model, tb(raw + step * direction))
        - mf.internal_energy(model, tb(raw - step * direction))
    ) / (2 * step)
