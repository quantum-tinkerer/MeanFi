...
class MumpsSelectedEntryPattern:
    size: int
    indptr: np.ndarray
    indices: np.ndarray
    rows: np.ndarray
    ...
    def entry_map(self, values: np.ndarray) -> dict[tuple[int, int], complex]:
        return {
...
def build_selected_entry_pattern(
    *,
    size: int,
    rows: np.ndarray,
    cols: np.ndarray,
    ...
    return MumpsSelectedEntryPattern(
        size=int(size),
        indptr=indptr,
        indices=indices,
        rows=pattern_rows,
...
class SelectedInverseFactorization:
    ...
    def selected_inverse(self, pattern: MumpsSelectedEntryPattern) -> np.ndarray:
        if pattern.nnz == 0:
