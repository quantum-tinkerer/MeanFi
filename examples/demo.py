...
if __name__ == "__main__":
    ...
    model = meanfi.Model(h_0, h_int, filling=2.0, kT=0.05)
    guess = meanfi.guess_tb(frozenset(h_int), ndof=4)

    solution = meanfi.solver(
        model,
        guess,
        integration=meanfi.AdaptiveQuadrature(density_matrix_tol=1e-6),
        scf=meanfi.AndersonMixing(M=0, max_iterations=80),
        scf_tol=1e-6,
    )

    h_mf = meanfi.add_tb(h_0, solution.mf)
    print(f"optimizer: {solution.info.optimizer}")
