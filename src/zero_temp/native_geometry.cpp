...
std::shared_ptr<NativeGeometry> NativeGeometry::root(
    size_t ndim,
...
NativeGeometry::NativeGeometry(size_t ndim, std::int64_t root_subcells_per_axis, double tol)
    : ndim_(ndim), root_subcells_per_axis_(root_subcells_per_axis), tol_(tol) {}

nb::ndarray<nb::numpy, double> NativeGeometry::vertices_array() const {
    return make_array(std::vector<double>(vertices_), {n_vertices(), ndim_});
}

nb::ndarray<nb::numpy, std::int64_t> NativeGeometry::active_simplex_ids() const {
    return make_array(
...
nb::ndarray<nb::numpy, std::int64_t> NativeGeometry::simplex_vertex_ids(
...
nb::ndarray<nb::numpy, double> NativeGeometry::simplex_points(std::int64_t simplex_id) const {
    std::vector<double> points = simplex_points_flat(simplex_id);
    return make_array(std::move(points), {ndim_ + 1, ndim_});
}

double NativeGeometry::simplex_volume(std::int64_t simplex_id) {
...
nb::ndarray<nb::numpy, std::int64_t> NativeGeometry::ensure_children(std::int64_t simplex_id) {
    const auto &children = ensure_children_impl(simplex_id);
    return make_array(std::vector<std::int64_t>(children), {children.size()});
}

nb::ndarray<nb::numpy, std::int64_t> NativeGeometry::descendant_leaves(
    std::int64_t simplex_id,
    std::int64_t levels
) {
...
nb::tuple NativeGeometry::refine(Int1D marked_ids) {
...
const std::vector<std::int64_t> &NativeGeometry::simplex_vertex_ids_vector(
    std::int64_t simplex_id
) const {
    return simplex_record(simplex_id).vertex_ids;
}

void NativeGeometry::build_root() {
...
std::int64_t NativeGeometry::get_or_add_vertex(const std::vector<double> &point) {
...
std::int64_t NativeGeometry::add_simplex(
...
const NativeSimplexRecord &NativeGeometry::simplex_record(std::int64_t simplex_id) const {
...
std::vector<double> NativeGeometry::simplex_points_flat(std::int64_t simplex_id) const {
    const auto &record = simplex_record(simplex_id);
...
void NativeGeometry::ensure_volume_cache_size() {
    if (simplex_volume_cache_.size() < simplex_records_.size()) {
        simplex_volume_cache_.resize(simplex_records_.size(), -1.0);
    }
}

double NativeGeometry::simplex_volume_impl(std::int64_t simplex_id) const {
    if (ndim_ == 0) {
        return 1.0;
    }
    return simplex_volume_from_flat(simplex_points_flat(simplex_id), ndim_ + 1, ndim_);
}

std::pair<std::int64_t, std::int64_t> NativeGeometry::longest_edge(std::int64_t simplex_id) const {
    if (ndim_ == 0) {
...
const std::vector<std::int64_t> &NativeGeometry::ensure_children_impl(std::int64_t simplex_id) {
...
void NativeGeometry::descendant_leaves_impl(
    std::int64_t simplex_id,
    std::int64_t levels,
...
NativeRefinementBatch NativeGeometry::refine_marked(const std::vector<std::int64_t> &marked_ids) {
