...
NativeTightBindingModel::NativeTightBindingModel(Int2D keys, Complex3D matrices) {
    ndim_ = keys.shape(1);
...
nb::ndarray<nb::numpy, std::int64_t> NativeTightBindingModel::keys_array() const {
    return make_array(std::vector<std::int64_t>(keys_), {nterms_, ndim_});
}

nb::ndarray<nb::numpy, std::complex<double>> NativeTightBindingModel::matrices_array() const {
    return make_array(
...
nb::ndarray<nb::numpy, std::complex<double>> NativeTightBindingModel::evaluate_many(
    Float2D points
) const {
...
nb::ndarray<nb::numpy, std::complex<double>> NativeTightBindingModel::evaluate_point(
    Float1D point
) const {
...
Eigen::MatrixXcd NativeTightBindingModel::evaluate_point_raw(const double *point) const {
...
double NativeTightBindingModel::spectral_bound() const {
    double bound = 0.0;
    for (size_t term = 0; term < nterms_; ++term) {
